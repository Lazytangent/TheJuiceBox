...
const SearchPage = () => {
  ...
  const similar = drinks.filter((drink) =>
  ...
  useEffect(() => {
    if (!similar.length) {
      dispatch(grabDrinks(query)).catch(() => setBadQuery(true));
    }
  ...
  return (
    <div className="tw-max-w-7xl tw-mx-auto tw-p-2 tw-min-h-screen">
      <h2 className="tw-text-5xl tw-text-clouds tw-font-semibold tw-text-center">
        Search Results
      </h2>
      {badQuery && (
        <h4 className="tw-text-3xl tw-text-clouds tw-text-center tw-mt-5">
          No Results Found
        </h4>
      )}
      {similar.map((drink) => (
